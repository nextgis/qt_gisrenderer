    ...
    : public QOffscreenSurface
{
  ...
  public:
    ...
    /// @brief Use bufferSize() instead size() for get a size of a surface buffer.
    /// We can't override size() as it is not virtual.
    ...
    /// @brief Resize surface buffer to size with width and height.
